...
// Static sample-report PDFs the user generates from real scans and drops
// into public/samples/. The landing HEAD-checks for each on mount and only
// surfaces a link for one that actually exists — so this ships safely before
// the PDFs are added (section stays hidden, no broken links) and self-
// activates the moment a file lands at the matching path.
const SAMPLE_FIELD_SCAN_PDF = "/samples/field-scan-sample.pdf";
const SAMPLE_FULL_ANALYSIS_PDF = "/samples/full-analysis-sample.pdf";
...
export default function WelcomeLanding({ onSelectMode }: Props) {
  ...
  const [samples, setSamples] = React.useState<{ field: boolean; full: boolean }>({
    field: false,
    full: false,
  });
  ...
  React.useEffect(() => {
    ...
    Promise.all([exists(SAMPLE_FIELD_SCAN_PDF), exists(SAMPLE_FULL_ANALYSIS_PDF)]).then(
      ([field, full]) => {
        if (!cancelled) setSamples({ field, full });
      }
    );
    return () => {
      cancelled = true;
  ...
  return (
    ...
    >
      ...
      {(samples.field || samples.full) && (
        <div style={{ marginTop: isCompact ? 28 : 40, textAlign: "center" }}>
          <div style={{ fontSize: isCompact ? 16 : 17, fontWeight: 700, color: DARK }}>
          ...
          >
            Generated by Proof Sleuth from actual photos — nothing staged.
          </p>
          <div
            style={{
              display: "flex",
          ...
          >
            {samples.field && (
              <SampleLink href={SAMPLE_FIELD_SCAN_PDF} label="Field Scan example (PDF)" />
            )}
            {samples.full && (
              <SampleLink href={SAMPLE_FULL_ANALYSIS_PDF} label="Full Analysis example (PDF)" />
            )}
          </div>
        </div>
      )}
